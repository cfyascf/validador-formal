...
TRANSITIONS = {
    ...
    ("q_start", "0"): ("q_seek_right_0", "X", "R"),
    ("q_start", "1"): ("q_seek_right_1", "Y", "R"),
    ("q_start", BLANK): (ACCEPT_STATE, BLANK, "S"),

    ("q_seek_right_0", "0"): ("q_seek_right_0", "0", "R"),
    ("q_seek_right_0", "1"): ("q_seek_right_0", "1", "R"),
    ("q_seek_right_0", "X"): ("q_seek_right_0", "X", "R"),
    ("q_seek_right_0", "Y"): ("q_seek_right_0", "Y", "R"),
    ("q_seek_right_0", BLANK): ("q_check_0", BLANK, "L"),

    ("q_seek_right_1", "0"): ("q_seek_right_1", "0", "R"),
    ("q_seek_right_1", "1"): ("q_seek_right_1", "1", "R"),
    ("q_seek_right_1", "X"): ("q_seek_right_1", "X", "R"),
    ("q_seek_right_1", "Y"): ("q_seek_right_1", "Y", "R"),
    ("q_seek_right_1", BLANK): ("q_check_1", BLANK, "L"),

    ("q_check_0", "X"): ("q_check_0", "X", "L"),
    ("q_check_0", "Y"): ("q_check_0", "Y", "L"),
    ("q_check_0", "0"): ("q_return_left", "X", "L"),
    ("q_check_0", "1"): (REJECT_STATE, "1", "S"),
    ("q_check_0", BLANK): (ACCEPT_STATE, BLANK, "S"),

    ("q_check_1", "X"): ("q_check_1", "X", "L"),
    ("q_check_1", "Y"): ("q_check_1", "Y", "L"),
    ("q_check_1", "1"): ("q_return_left", "Y", "L"),
    ("q_check_1", "0"): (REJECT_STATE, "0", "S"),
    ("q_check_1", BLANK): (ACCEPT_STATE, BLANK, "S"),
...
def recognize(text: str) -> dict:
    if not validate_alphabet(text):
        return {
            ...
            "trace": [],
            "reason": "A entrada deve conter apenas 0 e 1.",
        }
    ...
    return {
        ...
        "final_state": state,
        "trace": trace,
        "reason": "A fita foi verificada como palindromo." if state == ACCEPT_STATE else "A maquina encontrou uma extremidade incompatível.",
    }
...
def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print('Uso: python src/recursiva_palindromo.py "0110"')
        return 1
